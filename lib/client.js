window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		/** Move one queue entry up (-1) or down (+1) within its own session's slice of the
		* global send order — so reordering stays correct whether the dock shows only the
		* current session or every session. Returns true when moved. */
		function moveHold(id, delta) {
			var holds = readHolds();
			var from = holds.findIndex((h) => h.id === id);
			if (from < 0) return false;
			var sessionId = holds[from].sessionId;
			var peers = [];
			for (var i = 0; i < holds.length; i++) if (holds[i].sessionId === sessionId) peers.push(i);
			var p = peers.indexOf(from);
			var q = p + delta;
			if (q < 0 || q >= peers.length) return false;
			var to = peers[q];
		...
		//#region draggable dock window position
		var DOCK_POS_KEY = "dsh.peakGate.dockPos.v1";
		var DOCK_WIDTH = 380;
		/** Clamp a dock position so the window stays on screen. */
		function clampDockPos(pos, viewport, width) {
			var vw = viewport?.width ?? (typeof window.innerWidth === "number" ? window.innerWidth : 1280);
			var vh = viewport?.height ?? (typeof window.innerHeight === "number" ? window.innerHeight : 800);
			var w = typeof width === "number" ? width : DOCK_WIDTH;
			var maxX = Math.max(0, vw - w - 12);
			var maxY = Math.max(0, vh - 60);
			return { x: Math.min(Math.max(0, pos.x), maxX), y: Math.min(Math.max(0, pos.y), maxY) };
		}
		/** Saved dock position, or null when the user never dragged it (use the CSS default). */
		function loadDockPos() {
			try {
				var raw = window.localStorage.getItem(DOCK_POS_KEY);
				if (raw === null) return null;
				var parsed = JSON.parse(raw);
				if (parsed === null || typeof parsed !== "object" || typeof parsed.x !== "number" || typeof parsed.y !== "number") return null;
				return clampDockPos(parsed);
			} catch {
				return null;
			}
		}
		function saveDockPos(pos) {
			try {
				window.localStorage.setItem(DOCK_POS_KEY, JSON.stringify(clampDockPos(pos)));
			} catch {}
		}
		...
		var DOCK_SIZE_KEY = "dsh.peakGate.dockSize.v1";
		var DOCK_SIZE_MIN = { width: 260, height: 200 };
		var DOCK_SIZE_DEFAULT = { width: 300, height: 260 };
		function clampDockSize(size, viewport) {
			var vw = viewport?.width ?? (typeof window.innerWidth === "number" ? window.innerWidth : 1280);
			var vh = viewport?.height ?? (typeof window.innerHeight === "number" ? window.innerHeight : 800);
			return {
				width: Math.min(Math.max(DOCK_SIZE_MIN.width, size.width), Math.max(DOCK_SIZE_MIN.width, vw - 24)),
				height: Math.min(Math.max(DOCK_SIZE_MIN.height, size.height), Math.max(DOCK_SIZE_MIN.height, vh - 60))
			};
		}
		function loadDockSize() {
			try {
				var raw = window.localStorage.getItem(DOCK_SIZE_KEY);
				if (raw === null) return null;
				var parsed = JSON.parse(raw);
				if (parsed === null || typeof parsed !== "object" || typeof parsed.width !== "number" || typeof parsed.height !== "number") return null;
				return clampDockSize(parsed);
			} catch {
				return null;
			}
		}
		function saveDockSize(size) {
			try {
				window.localStorage.setItem(DOCK_SIZE_KEY, JSON.stringify(clampDockSize(size)));
			} catch {}
		}
		//#endregion

		//#region gate + queue + toast state
		var gateStore = createStore({ pending: null, toast: null, queue: false, suggestion: null });
		...
		* Returns true when the send was handed to the composer. */
		...
		/** Try to send one held message now that we are in off-peak. Every hold carries its own
		* copied text (explicit), so any number of messages can wait in parallel. Never clobbers
		* a draft the user is actively writing — that hold is skipped and keeps waiting. */
		function tryAutoSend(ctx, hold) {
			...
			// Restore this message's own text and submit it. Skip (keep waiting) when the
			// user is typing something else in the target session.
		...
		var CSS = [
			...
			".dsh-pg-fab{position:fixed;z-index:1800;width:44px;height:44px;right:18px;bottom:24px;border-radius:50%;background:var(--dsw-alias-state-warn-primary);color:#1c1200;border:none;box-shadow:var(--dsw-shadow-lv2);cursor:grab;place-items:center;padding:0;display:grid;touch-action:none;user-select:none;transition:transform .12s ease,box-shadow .12s ease}",
			".dsh-pg-fab:hover{transform:translateY(-1px);box-shadow:var(--dsw-shadow-lv2),0 4px 14px rgba(0,0,0,.35)}",
			".dsh-pg-fab:active{cursor:grabbing}",
			".dsh-pg-fab-icon{place-items:center;display:grid}",
			".dsh-pg-fab-badge{position:absolute;top:-4px;right:-4px;min-width:18px;height:18px;padding:0 5px;border-radius:999px;background:var(--dsw-alias-state-error-primary, #e5484d);color:#fff;font-size:11px;font-weight:600;line-height:18px;text-align:center;box-shadow:0 0 0 2px var(--dsw-specific-input-major)}",
			".dsh-pg-wrap{position:fixed;z-index:1800;width:300px;height:260px;right:18px;bottom:24px}",
			".dsh-pg-window{width:100%;height:100%;background:var(--dsw-specific-input-major);border:1px solid var(--dsw-alias-border-l2-darkmode-thin);border-radius:14px;box-shadow:var(--dsw-shadow-lv2);overflow:hidden;display:flex;flex-direction:column}",
			".dsh-pg-rz{position:absolute;z-index:3;touch-action:none}",
			".dsh-pg-rz-n{top:-3px;left:10px;right:10px;height:7px;cursor:ns-resize}",
			".dsh-pg-rz-s{bottom:-3px;left:10px;right:10px;height:7px;cursor:ns-resize}",
			".dsh-pg-rz-e{right:-3px;top:10px;bottom:10px;width:7px;cursor:ew-resize}",
			".dsh-pg-rz-w{left:-3px;top:10px;bottom:10px;width:7px;cursor:ew-resize}",
			".dsh-pg-rz-ne{top:-4px;right:-4px;width:12px;height:12px;cursor:nesw-resize}",
			".dsh-pg-rz-nw{top:-4px;left:-4px;width:12px;height:12px;cursor:nwse-resize}",
			".dsh-pg-rz-se{bottom:-4px;right:-4px;width:12px;height:12px;cursor:nwse-resize}",
			".dsh-pg-rz-sw{bottom:-4px;left:-4px;width:12px;height:12px;cursor:nesw-resize}",
			".dsh-pg-hheader{box-sizing:border-box;width:100%;height:40px;color:var(--dsw-alias-label-primary);display:flex;align-items:center;gap:10px;padding:4px 6px 4px 14px;cursor:grab;user-select:none;touch-action:none}",
			".dsh-pg-hheader:active{cursor:grabbing}",
			...
			".dsh-pg-hcollapse{width:28px;height:28px;flex:none;color:var(--dsw-alias-label-tertiary);cursor:pointer;background:0 0;border:none;border-radius:7px;place-items:center;padding:0;display:grid;transition:background .1s ease,color .1s ease}",
			".dsh-pg-hcollapse:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}",
			".dsh-pg-htoolbar{flex:none;display:flex;align-items:center;justify-content:space-between;gap:8px;padding:2px 10px 0 14px}",
			".dsh-pg-htoolbar-hint{color:var(--dsw-alias-label-caption, var(--dsw-alias-label-tertiary));font-size:11px;line-height:16px}",
			".dsh-pg-htools{flex:none;display:flex;align-items:center;gap:6px}",
			".dsh-pg-hscope{height:24px;font:inherit;font-size:11px;line-height:16px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:var(--dsw-alias-bg-module-platform);border:1px solid var(--dsw-alias-border-l2);border-radius:999px;padding:0 10px;transition:color .1s ease,border-color .1s ease,background .1s ease}",
			".dsh-pg-hscope:hover{color:var(--dsw-alias-label-primary)}",
			".dsh-pg-hscope-on{color:var(--dsw-alias-state-warn-primary);border-color:var(--dsw-alias-state-warn-secondary);background:var(--dsw-alias-state-warn-tertiary)}",
			".dsh-pg-sug{position:fixed;z-index:2147483002;width:min(400px,calc(100vw - 32px));background:var(--dsw-specific-input-major);border:1px solid var(--dsw-alias-border-l2-darkmode-thin);border-radius:14px;box-shadow:var(--dsw-shadow-lv2);padding:8px;display:flex;flex-direction:column;gap:4px}",
			".dsh-pg-sug-title{color:var(--dsw-alias-state-warn-primary);font-size:12px;font-weight:500;line-height:18px;padding:2px 6px 4px}",
			".dsh-pg-sug-item{width:100%;display:flex;align-items:baseline;gap:10px;color:var(--dsw-alias-label-primary);text-align:left;cursor:pointer;background:0 0;border:none;border-radius:9px;padding:7px 10px;font:inherit;transition:background .1s ease}",
			".dsh-pg-sug-item:hover{background:var(--dsw-alias-interactive-bg-hover)}",
			".dsh-pg-sug-label{flex:none;min-width:64px;font-size:13px;font-weight:500;line-height:20px;color:var(--dsw-alias-state-warn-primary)}",
			".dsh-pg-sug-desc{color:var(--dsw-alias-label-secondary);font-size:12px;line-height:18px}",
			".dsh-pg-hpanel{flex:1;min-height:0;width:100%;padding:6px 8px 8px;position:relative;overflow:hidden;display:flex;flex-direction:column;gap:6px}",
			...
			".dsh-pg-hlist{flex:1;min-height:0;overflow-y:auto;flex-direction:column;display:flex;gap:2px;padding-top:2px}",
		...
		var zh = {
			...
			"cmd.holdUsage": "用法：/peakgate hold 想发送的内容",
			...
			"cmd.help": "待发送队列指令（空闲时段半价自动发送）：\n/peakgate hold 想发的内容 —— 加入队列，空闲时段自动发出\n/peakgate list —— 打开队列窗口\n/peakgate remove 序号 —— 删除队列中的一条\n/peakgate cancel —— 清空队列\n输入 /peakgate 也会弹出这些选项",
			"sug.title": "待发送队列指令（空闲时段半价发送）：",
			"sug.hold": "排队发送",
			"sug.holdDesc": "把消息存到队列，空闲时段自动发出",
			"sug.list": "查看队列",
			"sug.listDesc": "打开队列窗口（排序/编辑/删除）",
			"sug.remove": "删除某条",
			"sug.removeDesc": "按序号删除队列中的一条",
			"sug.cancel": "清空队列",
			"sug.cancelDesc": "清空所有排队消息",
			...
			"queue.closeAria": "关闭",
			"dock.title": "待发送队列 ({n})",
			"dock.empty": "队列为空。高峰时段发送消息时，在确认卡片上点「等到空闲时段（自动发送）」，消息就会在这里排队，空闲时段开始后自动以半价发出。",
			"dock.up": "上移（提前发送）",
			"dock.down": "下移（延后发送）",
			"dock.edit": "修改文本",
			"dock.save": "保存",
			...
			"dock.open": "打开待发送队列",
			"dock.collapse": "收起",
			"dock.resizeHint": "拖动边框调整大小",
			"dock.scopeAll": "全部会话",
			"dock.scopeCurrent": "仅当前会话",
			"dock.emptyCurrent": "当前会话没有排队消息（其他会话有 {n} 条，可点“全部会话”查看）"
		};
		var en = {
			...
			"cmd.help": "Send-queue commands (auto-send at off-peak, half price):\n/peakgate hold <message> — queue a message for off-peak auto-send\n/peakgate list — open the queue window\n/peakgate remove <index> — remove one queued item\n/peakgate cancel — clear the queue\nTyping /peakgate also shows these options",
			"sug.title": "Send-queue commands (off-peak half price):",
			"sug.hold": "Queue message",
			"sug.holdDesc": "Hold a message and auto-send it at off-peak",
			"sug.list": "Open queue",
			"sug.listDesc": "Open the queue window (reorder / edit / delete)",
			"sug.remove": "Remove one",
			"sug.removeDesc": "Remove a queued item by index",
			"sug.cancel": "Clear queue",
			"sug.cancelDesc": "Clear all queued messages",
			...
			"dock.empty": "The queue is empty. When you send during peak hours, choose “Wait for off-peak (auto-send)” on the confirmation card — the message is queued here and sent automatically at half price when off-peak starts.",
			...
			"dock.dragHint": "Drag the title bar to move the window",
			"dock.open": "Open send queue",
			"dock.collapse": "Collapse",
			"dock.resizeHint": "Drag the edges to resize",
			"dock.scopeAll": "All sessions",
			"dock.scopeCurrent": "Current session only",
			"dock.emptyCurrent": "No queued messages in this session ({n} in others — switch to “All sessions” to see them)"
		};
		//#endregion
		...
		/** "Wait for off-peak": optionally mute this peak segment, then copy the draft into the
		* send queue and clear the input — any number of messages can wait in parallel this way. */
		function deferAndHold(ctx, sessionId, mute) {
			...
			var holds = readHolds();
			holds.push({ id: nextHoldId(), sessionId, text, at: Date.now(), explicit: true });
			writeHolds(holds);
			try {
		...
		/** True when the draft is an incomplete /peakgate command that deserves an inline hint bar. */
		function suggestionFor(draft) {
			if (typeof draft !== "string") return false;
			var t = draft.trim();
			if (t.toLowerCase().indexOf(COMMAND_PREFIX.toLowerCase()) !== 0) return false;
			var cmd = commandFromDraft(t);
			if (cmd === null) return false;
			if (cmd.name === "help") return true;
			if ((cmd.name === "hold" || cmd.name === "remove") && cmd.arg === "") return true;
			return false;
		}
		/**
		...
		* Defaults to the current session's entries; a toolbar toggle shows every session. */
		function HoldQueueDock({ t, gateStore: gate, ctx, useSessions, sessionId }) {
			...
			var [pos, setPos] = react.useState(loadDockPos());
			var [size, setSize] = react.useState(loadDockSize());
			var [scope, setScope] = react.useState("current");
			var dragRef = react.useRef(null);
			var sizeRef = react.useRef(null);
			var panelRef = react.useRef(null);
			var wrapRef = react.useRef(null);
			var justDraggedRef = react.useRef(false);
			var allHolds = readHolds();
			var holds = scope === "all" || sessionId === void 0 ? allHolds : allHolds.filter((h) => h.sessionId === sessionId);
			var otherCount = allHolds.length - holds.length;
			var byId = {};
			try {
			...
			/** Shared drag handling for the floating button and the panel header.
			* Move/up listeners live on window (never setPointerCapture — capture would
			* retarget the trailing click away from inner buttons like the collapse ×). */
			var dragHandlersRef = react.useRef(null);
			var onDragPointerDown = (event) => {
				if (event.button !== 0) return;
				document.body.style.userSelect = "none";
				dragRef.current = { pointerId: event.pointerId, startX: event.clientX, startY: event.clientY, origX: pos?.x ?? 0, origY: pos?.y ?? 0, moved: false };
				var move = (ev) => {
					...
					drag.moved = true;
					setPos(clampDockPos({ x: drag.origX + dx, y: drag.origY + dy }));
				};
				var up = (ev) => {
					...
					if (drag.moved) {
						var dx = ev.clientX - drag.startX;
						var dy = ev.clientY - drag.startY;
						saveDockPos(clampDockPos({ x: drag.origX + dx, y: drag.origY + dy }));
			...
			react.useEffect(() => () => {
				var handlers = dragHandlersRef.current;
				if (handlers !== null) {
					window.removeEventListener("pointermove", handlers.move);
					window.removeEventListener("pointerup", handlers.up);
				}
				var resize = sizeRef.current;
				if (resize !== null) {
					window.removeEventListener("pointermove", resize.move);
					window.removeEventListener("pointerup", resize.up);
				}
			}, []);
			// Clicking anywhere outside the open panel collapses it back to the floating button.
			react.useEffect(() => {
				if (!expanded) return;
				var onDocPointerDown = (event) => {
					var wrap = wrapRef.current;
					var target = event.target;
					if (wrap === null || !(target instanceof Element)) return;
			...
			/** Resize from any edge/corner (n/s/e/w + diagonals), anchored on the opposite side.
			* Works regardless of CSS default (right/bottom) or dragged (left/top) positioning
			* by reading the live rect at pointerdown; on finish the panel switches to left/top. */
			var onResizePointerDown = (event, dir) => {
				if (event.button !== 0) return;
				...
				var move = (ev) => {
					var r = sizeRef.current;
					if (r === null || r.pointerId !== ev.pointerId) return;
					var dx = ev.clientX - r.startX;
					var dy = ev.clientY - r.startY;
					var x = r.rect.x;
					var y = r.rect.y;
					var w = r.rect.width;
					var h = r.rect.height;
					if (r.dir.indexOf("e") !== -1) w = r.rect.width + dx;
					if (r.dir.indexOf("s") !== -1) h = r.rect.height + dy;
					...
					var cs = clampDockSize({ width: w, height: h });
					if (r.dir.indexOf("w") !== -1) x = r.rect.x + r.rect.width - cs.width;
					if (r.dir.indexOf("n") !== -1) y = r.rect.y + r.rect.height - cs.height;
					var cp = clampDockPos({ x: x, y: y }, null, cs.width);
					setSize(cs);
					setPos(cp);
				};
				var up = (ev) => {
				...
				sizeRef.current = { pointerId: event.pointerId, startX: event.clientX, startY: event.clientY, rect: { x: rect.x, y: rect.y, width: rect.width, height: rect.height }, dir };
				window.addEventListener("pointermove", move);
				window.addEventListener("pointerup", up);
			};
			/** Click on the collapsed floating button: expand the panel (swallow the trailing click after a drag). */
			var onFabClick = () => {
				...
				return createElement(
					...
					isEditing
						...
						: createElement(
								...
								createElement("div", { className: "dsh-pg-hmeta" }, byId[hold.sessionId]?.displayTitle ?? byId[hold.sessionId]?.title ?? "")
						  ),
					createElement(
						"div",
			...
			var floatStyle = pos === null ? null : { left: pos.x + "px", top: pos.y + "px", transform: "none" };
			if (!expanded) {
				// Collapsed: a tiny draggable floating button with a queue badge.
				return createElement(
					"button",
					{
						type: "button",
						className: "dsh-pg-fab",
						"data-dsh-hold-dock": "",
						style: floatStyle,
						title: t("dock.title", { n: holds.length }),
						"aria-label": t("dock.open"),
						onPointerDown: onDragPointerDown,
						onClick: onFabClick
					},
					createElement("span", { className: "dsh-pg-fab-icon", "aria-hidden": true },
						createElement("svg", { viewBox: "0 0 16 16", width: "16", height: "16", fill: "none", stroke: "currentColor", strokeWidth: "1.4", strokeLinecap: "round", strokeLinejoin: "round", "aria-hidden": true }, createElement("path", { d: "M2 4h12M4 8h8M6 12h4" }))),
					holds.length > 0 &&
						createElement("span", { className: "dsh-pg-fab-badge" }, String(holds.length))
				);
			}
			var panelSize = size ?? DOCK_SIZE_DEFAULT;
			var wrapStyle = { width: panelSize.width + "px", height: panelSize.height + "px", ...(pos === null ? {} : { left: pos.x + "px", top: pos.y + "px", transform: "none" }) };
			var resizeDirs = ["n", "s", "e", "w", "ne", "nw", "se", "sw"];
			return createElement(
				"div",
				{ className: "dsh-pg-wrap", "data-dsh-hold-dock": "", style: wrapStyle, ref: wrapRef },
				createElement(
					"div",
					{ className: "dsh-pg-window", ref: panelRef },
					createElement(
						"div",
						{
						...
						createElement(
							...
							{
								type: "button",
								className: "dsh-pg-hcollapse",
								"aria-label": t("dock.collapse"),
								title: t("dock.collapse"),
								onClick: closeQueue
							},
							createElement("svg", { viewBox: "0 0 16 16", width: "14", height: "14", fill: "none", stroke: "currentColor", strokeWidth: "1.5", strokeLinecap: "round", "aria-hidden": true }, createElement("path", { d: "M4 4l8 8M12 4l-8 8" }))
						)
					),
					createElement(
						"div",
					...
					createElement(
						"div",
						{ className: "dsh-pg-htoolbar" },
						createElement("span", { className: "dsh-pg-htoolbar-hint" }, t("dock.dragHint")),
						createElement(
							"div",
							{ className: "dsh-pg-htools" },
							sessionId !== void 0 &&
								createElement(
									"button",
									{
										type: "button",
										className: "dsh-pg-hscope" + (scope === "all" ? " dsh-pg-hscope-on" : ""),
										"aria-label": scope === "all" ? t("dock.scopeCurrent") : t("dock.scopeAll"),
										title: scope === "all" ? t("dock.scopeCurrent") : t("dock.scopeAll"),
										onClick: () => setScope((s) => (s === "all" ? "current" : "all"))
									},
									scope === "all" ? t("dock.scopeCurrent") : t("dock.scopeAll")
								)
						)
					),
					holds.length === 0
						? createElement(
								...
								createElement(
									"span",
									null,
									scope === "current" && otherCount > 0 ? t("dock.emptyCurrent", { n: otherCount }) : t("dock.empty")
								)
						  )
				...
				resizeDirs.map((dir) =>
					createElement("span", {
						key: dir,
						className: "dsh-pg-rz dsh-pg-rz-" + dir,
						"data-rz": dir,
		...
		/** Inline command hint shown while typing an incomplete /peakgate command. */
		function SuggestionBar({ t, ctx, suggestion, onHide }) {
			var pick = (draft) => {
				var sessionId = currentSessionId(ctx);
				if (sessionId !== void 0) {
					try {
						ctx.sessions.provideInfo(sessionId)?.props?.inputActions?.setDraft?.(draft);
					} catch {}
				}
				document.querySelector("[data-composer-seat] textarea")?.focus();
				onHide();
			};
			var items = [
				{ key: "hold", label: t("sug.hold"), desc: t("sug.holdDesc"), action: () => pick("/peakgate hold ") },
				{ key: "list", label: t("sug.list"), desc: t("sug.listDesc"), action: () => { onHide(); openQueue(); } },
				{ key: "remove", label: t("sug.remove"), desc: t("sug.removeDesc"), action: () => pick("/peakgate remove ") },
				{ key: "cancel", label: t("sug.cancel"), desc: t("sug.cancelDesc"), action: () => { onHide(); writeHolds([]); showToast("cmd.cancelDone", "success"); } }
			];
			return createElement(
				"div",
				{ className: "dsh-pg-sug", style: { left: suggestion.x + "px", top: suggestion.y + "px" } },
				createElement("div", { className: "dsh-pg-sug-title" }, t("sug.title")),
				items.map((item) =>
					createElement(
						"button",
						{ type: "button", className: "dsh-pg-sug-item", key: item.key, onClick: item.action },
						createElement("span", { className: "dsh-pg-sug-label" }, item.label),
						createElement("span", { className: "dsh-pg-sug-desc" }, item.desc)
					)
				)
			);
		}
		/** Portal root: renders the gate card, the queue hint bar, and/or toast. */
		function GatePortal({ ctx, t }) {
			...
			var hideSuggestion = () => {
				var cur = gateStore.getSnapshot();
				if (cur.suggestion !== null) gateStore.set({ ...cur, suggestion: null });
			};
			var nodes = [];
			if (pending !== null) {
			...
			if (suggestion !== null) {
				nodes.push(createElement(SuggestionBar, { key: "sug", t: t, ctx: ctx, suggestion: suggestion, onHide: hideSuggestion }));
			}
			if (toast !== null) {
		...
		*/
		function apply(ctx) {
			...
			var onKeyDown = (event) => {
				...
				// A confirmation card is already open: Enter now confirms the primary
				// action ("send now") — the user pressed Enter again deliberately.
				var pendingNow = gateStore.getSnapshot().pending;
				if (pendingNow !== null) {
					event.preventDefault();
					event.stopPropagation();
					consentAndSubmit(ctx, pendingNow.sessionId, false);
					return;
				}
			...
			// Inline command hint while typing an incomplete /peakgate command in the composer.
			var onInputCapture = (event) => {
				var target = event.target;
				if (!(target instanceof Element)) return;
				if (target.tagName !== "TEXTAREA") return;
				if (target.closest("[data-composer-seat]") === null) return;
				var value = typeof target.value === "string" ? target.value : "";
				var show = suggestionFor(value);
				var cur = gateStore.getSnapshot();
				if (!show) {
					if (cur.suggestion !== null) gateStore.set({ ...cur, suggestion: null });
					return;
				...
				gateStore.set({ ...cur, suggestion: { x: rect.left, y: rect.bottom + 8 } });
			};
