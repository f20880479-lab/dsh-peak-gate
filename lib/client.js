window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		* current session or every session. Returns true when moved. */
		...
		function clampDockPos(pos, viewport) {
			...
			var maxX = Math.max(0, vw - DOCK_WIDTH - 12);
		...
		* Returns true when the send was handed to the composer. */
		...
		var CSS = [
			...
			".dsh-pg-window{position:fixed;z-index:1800;width:300px;height:260px;right:18px;bottom:24px;background:var(--dsw-specific-input-major);border:1px solid var(--dsw-alias-border-l2-darkmode-thin);border-radius:14px;box-shadow:var(--dsw-shadow-lv2);overflow:hidden;display:flex;flex-direction:column}",
			".dsh-pg-resize{position:absolute;right:0;bottom:0;width:18px;height:18px;cursor:nwse-resize;z-index:2;background:linear-gradient(135deg,transparent 50%,var(--dsw-alias-label-tertiary) 50%,var(--dsw-alias-label-tertiary) 57%,transparent 57%);opacity:.6;touch-action:none}",
			".dsh-pg-resize:hover{opacity:1}",
		...
		* Defaults to the current session's entries; a toolbar toggle shows every session. */
		function HoldQueueDock({ t, gateStore: gate, ctx, useSessions, sessionId }) {
			...
			* retarget the trailing click away from inner buttons like the collapse ×). */
			...
			/** Resize the panel from its bottom-right handle (window-level listeners, like drag). */
			var onResizePointerDown = (event) => {
				if (event.button !== 0) return;
				var base = size ?? DOCK_SIZE_DEFAULT;
				var move = (ev) => {
					var r = sizeRef.current;
					if (r === null || r.pointerId !== ev.pointerId) return;
					var next = clampDockSize({ width: r.startWidth + (ev.clientX - r.startX), height: r.startHeight + (ev.clientY - r.startY) });
					setSize(next);
				};
				var up = (ev) => {
					window.removeEventListener("pointermove", move);
				...
				sizeRef.current = { pointerId: event.pointerId, startX: event.clientX, startY: event.clientY, startWidth: base.width, startHeight: base.height };
				window.addEventListener("pointermove", move);
				window.addEventListener("pointerup", up);
			};
			...
			var panelStyle = { width: panelSize.width + "px", height: panelSize.height + "px", ...(pos === null ? {} : { left: pos.x + "px", top: pos.y + "px", transform: "none" }) };
			return createElement(
				"div",
				{ className: "dsh-pg-window", "data-dsh-hold-dock": "", style: panelStyle },
				createElement(
					"div",
					{
