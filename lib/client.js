	...
	factory: (require) => {
		...
		* Returns true when the send was handed to the composer. */
		...
		var CSS = [
			...
			".dsh-pg-window{position:fixed;z-index:1800;width:min(380px,calc(100vw - 24px));right:18px;bottom:24px;background:var(--dsw-specific-input-major);border:1px solid var(--dsw-alias-border-l2-darkmode-thin);border-radius:14px;box-shadow:var(--dsw-shadow-lv2);overflow:hidden;display:flex;flex-direction:column}",
		...
		*/
		...
		function HoldQueueDock({ t, gateStore: gate, ctx, useSessions }) {
			...
			* (pointer events; a small move threshold separates a click from a drag). */
			...
			return createElement(
				"div",
				{ className: "dsh-pg-window", "data-dsh-hold-dock": "", style: floatStyle },
				createElement(
					"div",
					{
